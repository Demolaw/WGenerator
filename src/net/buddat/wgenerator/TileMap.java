...
public class TileMap
{
	...
	void dropDirt(final int dirtCount,
	              final int maxSlope,
	              final int maxDiagSlope,
	              final int maxDirtHeight,
	              final double cliffRatio,
	              final boolean landSlide,
					...
					for (int y = iy; y < iy + sizey; y++)
					{
						...
						if (x % mod == 0 && y % mod == 0)
						{
							...
							progress.update(progressValue,
							                (progress.getText().substring(0,
							                                              progress.getText().indexOf("(")) + "(" + predict + " secs)"));
						}
	...
	void plantBiome(int seedCount,
	                int growthIterations,
	                int density,
	                int[] growthRate,
	                boolean randomGrowth,
	                int maxBiomeSlope,
	                int minHeight,
	                int maxHeight,
	                Tile type,
	                ...
	                ProgressHandler progress)
	{
		...
		for (long killCount = 0; totalSeeds < seedCount; killCount++)
		{
			...
			{
				nextList = growBiome(nextList,
				                     type,
				                     density,
				                     randomGrowth ? randomRate : growthRate,
				                     maxBiomeSlope,
				                     minHeight,
				                     maxHeight,
				                     flowerType,
	...
	private ArrayList<Point> growBiome(ArrayList<Point> fromList,
	                                   Tile type,
	                                   int density,
	                                   int[] growthRate,
	                                   int maxBiomeSlope,
	                                   ...
	                                   int flowerPercent)
	{
		...
		{
			dirMod = (type.isTree() ? biomeRandom.nextInt(4 * density) + 3 : (type.isBush() ? biomeRandom.nextInt(2 * density) + 2 : biomeRandom.nextInt(
					density) + 1));

			if (biomeRandom.nextInt(100) < growthRate[0])
			{ //North
				Point nT = new Point(p.x, HeightMap.clamp((int) (p.getY() - dirMod), 0, mapSize - 1));
				if (setBiome(routeMap,
				             p,
				             nT,
				             maxBiomeSlope * dirMod,
				             type,
				             minHeight,
				             maxHeight,
				             flowerType,
			...
			if (biomeRandom.nextInt(100) < growthRate[1])
			{ //South
				Point nT = new Point(p.x, HeightMap.clamp((int) (p.getY() + dirMod), 0, mapSize - 1));
				if (setBiome(routeMap,
				             p,
			...
			if (biomeRandom.nextInt(100) < growthRate[2])
			{ //East
			...
			if (biomeRandom.nextInt(100) < growthRate[3])
			{ //West
				Point nT = new Point(HeightMap.clamp((int) (p.getX() - dirMod), 0, mapSize - 1), p.y);
				if (setBiome(routeMap,
				             p,
	...
	private boolean setBiome(HashMap<Point, Boolean> routeMap,
	                         Point from,
	                         Point to,
	                         int maxBiomeSlope,
	                         Tile type,
	...
	void importBiomeImage(BufferedImage biomesImage)
	{
		...
		if (biomesImage.getHeight() != heightMap.getMapSize() || biomesImage.getWidth() != heightMap.getMapSize())
		{
			JOptionPane.showMessageDialog(null,
			                              "The image size does not match your map size! " + biomesImage.getHeight(),
			                              "Error",
		...
		catch (Exception e)
		{
			e.printStackTrace();
			JOptionPane.showMessageDialog(null,
			                              "The map must be a 24-bit color image.",
			                              "Error",
			                              JOptionPane.ERROR_MESSAGE);
		}
	}
