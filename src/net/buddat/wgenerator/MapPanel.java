...
public class MapPanel extends JPanel
{
	...
	public void paintComponent(Graphics g)
	{
		...
		if (showGrid)
		{
			...
			{
				g.drawLine(imageX,
				           imageY + (int) (x * gridScale),
				           imageX + (int) (mapSize * scale),
				           (int) (imageY + x * gridScale));
			}
			for (int y = 0; y <= gridSize; y++)
			{
				g.drawLine((int) (imageX + y * gridScale),
				           imageY,
				           (int) (imageX + y * gridScale),
		...
		for (Point p : riverSeeds)
		{
			g.fillOval((int) (p.x * scale + imageX) - riverMarker / 2,
			           (int) (p.y * scale + imageY) - riverMarker / 2,
			           riverMarker,
			           riverMarker);
		}
		...
		if (showMarker)
		{
			...
			g.fillOval((int) ((markerOffsetX * scale) + imageX) - 4,
			           (int) ((markerOffsetY * scale) + imageY) - 4,
			           8,
			           8);
		}
	}
