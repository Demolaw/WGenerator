...
public final class MapData
{
	...
	public void setSurfaceTile(int x, int y, Tile tileType, short height)
	{
		...
		else if (tileType.isTree() || tileType.isBush())
		{
			throw new IllegalArgumentException(
					"Tile type is tree or bush: please use specialized methods to put them on map instead.");
		}
	...
	public void setGrass(int x, int y, GrassData.GrowthStage grassStage, GrassData.FlowerType flower)
	{
		...
		surfaceMesh.setTile(x,
		                    y,
		                    Tiles.encode(currentHeight,
		                                 (byte) currentType,
		                                 GrassData.encodeGrassTileData(grassStage, grassType, flower)));
	}
	...
	public BufferedImage createMapDump()
	{
		...
		for (int x = 0; x < lWidth; x++)
		{
			...
			for (int y = lWidth - 1; y >= 0; y--)
			{
				...
				float node2 = x == lWidth - 1 || y == lWidth - 1 ? node : (float) (getSurfaceHeight(x + 1 + xo,
				                                                                                    y + 1 + yo) / (Short.MAX_VALUE / 3.3f));
				...
				final int altTarget = y - (int) (Tiles.decodeHeight(surfaceMesh.getTile(x,
				                                                                        y)) * MAP_HEIGHT / 4 / (Short.MAX_VALUE / 3.3f));
	...
	public BufferedImage createTopographicDump(boolean showWater, short interval)
	{
		...
		for (int x = 0; x < lWidth; x++)
		{
			for (int y = lWidth - 1; y >= 0; y--)
			{
				...
				final short nearHeightNX = x == 0 ? height : Tiles.decodeHeight(surfaceMesh.getTile(x + xo - 1,
				                                                                                    y + yo));
				final short nearHeightNY = y == 0 ? height : Tiles.decodeHeight(surfaceMesh.getTile(x + xo,
				                                                                                    y + yo - 1));
				final short nearHeightX = x == lWidth - 1 ? height : Tiles.decodeHeight(surfaceMesh.getTile(x + xo + 1,
				                                                                                            y + yo));
				final short nearHeightY = y == lWidth - 1 ? height : Tiles.decodeHeight(surfaceMesh.getTile(x + xo,
				                                                                                            y + yo + 1));
				boolean isControur = checkContourLine(height, nearHeightNX, interval) || checkContourLine(height,
				                                                                                          nearHeightNY,
				                                                                                          interval) || checkContourLine(
						height,
						nearHeightX,
						interval) || checkContourLine(height, nearHeightY, interval);
